 ...
 * @copyright Copyright (C) 2017-2018, kevinhaig
...
function canuck_theme_page_tabs( $current = 'canuck_intro_page' ) {
	...
	foreach ( $tabs as $tab => $name ) {
		...
		echo "<a class='nav-tab$class' href='?page=canuck_theme_page&tab=$tab&_wp_nonce=$canuck_admin_nonce'>$name</a>"; // XSS OK.
	}
	echo '</h2>';
}
...
function canuck_theme_page_callback() {
	...
	<div class="canuck-wrap">
		...
		<?php
		if ( isset( $_GET['tab'] ) && isset( $_GET['_wp_nonce'] ) && false !== wp_verify_nonce( $_GET['_wp_nonce'], 'canuck-admin-nonce' ) ) {// WPCS: input var ok.
			canuck_theme_page_tabs( esc_html( wp_unslash( $_GET['tab'] ) ) ); // WPCS: input var ok, sanitization ok.
		} else {
		...
		<div id="canuck-page">
			<?php
			if ( isset( $_GET['page'] ) ) {
				$page_string = sanitize_text_field( wp_unslash( $_GET['page'] ) ); // WPCS: input var ok.
			}
			if ( 'themes.php' === $pagenow && 'canuck_theme_page' === $page_string ) { // WPCS: input var ok.
				if ( isset( $_GET['tab'] ) && isset( $_GET['_wp_nonce'] ) && false !== wp_verify_nonce( $_GET['_wp_nonce'], 'canuck-admin-nonce' ) ) { // WPCS: input var ok, sanitization ok.
					$tab = esc_html( wp_unslash( $_GET['tab'] ) ); // WPCS: input var ok, sanitization ok.
				} else {
...
function canuck_intro_setup_callback() {
	...
	<div class="katb_paypal"><?php esc_html_e( 'Show your appreciation!', 'canuck' ); ?>
		<br/><br/>
		<a href="https://canuckdemo.kevinsspace.ca/donate/" target="_blank"> <?php // url ok. ?>
			<img alt="<?php esc_attr_e( 'Donate Button with Credit Cards', 'canuck' ); ?>" src="https://www.paypalobjects.com/webstatic/en_US/btn/btn_donate_cc_147x47.png" /> <?php // url ok. ?>
		</a>
	</div>
	<p>
		<?php
		echo esc_html__( 'Author Site : ', 'canuck' ) . '<a href="//kevinsspace.ca" target="_blank" >kevinsspace.ca</a>&nbsp;&nbsp;&nbsp;'; // url ok.
		echo esc_html__( 'Demo Site : ', 'canuck' ) . '<a href="//cauckdemo.kevinsspace.ca/" target="_blank" >Canuck Demo</a>'; // url ok.
		?>
	</p>
	<?php
...
function canuck_quick_setup_callback() {
	...
	echo '<li>' . esc_html__( 'For example URL->"//www.facebook.com/yourlink" Link Text->"Facebook".', 'canuck' ) . '</li>'; // url ok.
