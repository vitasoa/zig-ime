...
function canuck_add_menu_cart_item_to_menus( $items, $args ) {
	...
	if ( 'canuck_primary' === $args->theme_location ) {
		$css_class = 'menu-item menu-item-type-cart menu-item-type-woocommerce-cart'; // translate ok.
		if ( is_cart() ) {
...
function canuck_menu_cart_item() {
	...
	$css_class  = 'canuck-menu-cart-total canuck-cart-total-' . intval( $cart_count ); // translate ok.
	if ( $cart_count ) {
		$url = wc_get_cart_url();
	} else {
