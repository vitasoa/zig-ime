...
if ( ! class_exists( 'queens_magazine_blog_About' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class queens_magazine_blog_About {
		...
		/**
		 * Singleton instance of queens_magazine_blog_About.
		 *
		 * @var queens_magazine_blog_About $instance queens_magazine_blog_About instance.
		 */
		private static $instance;

		/**
		 * Main queens_magazine_blog_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $importconfig ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof queens_magazine_blog_About ) ) {
				self::$instance = new queens_magazine_blog_About;
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : get_template_directory_uri() . '/inc/ocdi/images/logoqueens_magazine_blog.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://www.queens_magazine_blogthemes.com/';
		...
		/* translators: %s: get started */
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'queens-magazine-blog' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'queens-magazine-blog' ), $this->theme_name ) . '</a></p>' );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'queens_magazine_blog-about', get_template_directory_uri() . '/inc/ocdi/css/about.css', array(), '1.0.0' );				
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'queens-magazine-blog' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'queens-magazine-blog' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'queens-magazine-blog' );
									break;
							}
							?>
		...
		public function get_plugins( $plugin_folder = '' ) {
			if ( ! function_exists( 'get_plugins' ) ) {
				get_template_part('wp-admin/includes/plugin', 'queens-magazine-blog');
