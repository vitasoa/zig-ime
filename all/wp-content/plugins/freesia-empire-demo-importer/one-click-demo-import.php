...
add_action( 'freesiaempire_footer_column', 'freesiaempire_footer_column_section' );
function freesiaempire_import_files() {
  return array(
    array(
      'import_file_name'             => 'Import Demo Content',
      'categories'                   => array( 'Category 1', 'Category 2' ),
      'local_import_file'            => trailingslashit( plugin_dir_path( __FILE__ ) ).'inc/ocdi/freesiaempire.wordpress.xml',
      'local_import_widget_file'     => trailingslashit( plugin_dir_path( __FILE__ ) ).'inc/ocdi/freesia-empire-widgets.wie',
      'local_import_customizer_file' => trailingslashit( plugin_dir_path( __FILE__ ) ).'inc/ocdi/freesia-empire-export.dat',
      'import_notice'                => __( 'After you import this demo, you will have to setup the slider separately.', 'freesiaempire' ),
    ),
  );
}
add_filter( 'pt-ocdi/import_files', 'freesiaempire_import_files' );

function freesiaempire_after_import_setup() {
	// Assign menus to their locations.
	$main_menu = get_term_by( 'name', 'Main Menu', 'nav_menu' );
	$footer_menu = get_term_by( 'name', 'Add Social Icons Only', 'nav_menu' );
...
add_action( 'pt-ocdi/after_import', 'freesiaempire_after_import_setup' );
/**
