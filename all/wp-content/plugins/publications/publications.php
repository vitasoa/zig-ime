...
function wclRenderPublicationList()
{
	...
	while ($loop->have_posts()) {
		...
		$output .= '<p class="publication-text">';
		//$image = get_post_meta(get_the_ID(), "wcl_image", true);
		$image = get_the_post_thumbnail(get_the_ID(), array(105,105), array('class' => 'alignleft', 'style' => 'margin-top: 0px !important;'));
		...
		if (!empty($proceedings)) {
			$output .= 'in <i style="font-weight: bold;">' . $proceedings . '</i>, ';
		}
